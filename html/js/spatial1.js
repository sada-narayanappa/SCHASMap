...
/*
 * Use Law of Cosines to calculate distance (in km) between two points specified by latitude/longitude * (in numeric degrees).
 */
...
// if dLon over 180� take shorter rhumb across 180� meridian:
   ...
   var d = Math.sqrt(dLat*dLat + q*q*dLon*dLon); return d * R;
}
...
//
// this is very flexible on formats, allowing signed decimal degrees, or deg-min-sec suffixed by // compass direction (NSEW). A variety of separators are accepted (eg 3� 37' 09"W) or fixed-width // format without separators (eg 0033709W). Seconds and minutes may be omitted. (Minimal validation // is done).
...
Number.prototype.toPrecision1 = function(fig) { // override toPrecision method with one which displays if (this == 0) return 0; // trailing zeros in place of exponential notation
...
/**
 *
 * Pass two strings MyDistance(" 39,34, -104.343,", " 56, -103.343" )
 * lat, lon and lat2, lon2
 *
 * return Kilometers
 */
function MyDistance( d1, d2) {
   l1 = d1.split(",")
   l2 = d2.split(",")
   ...
   d = LatLon.distHaversine(lat1 , lon1,  lat2, lon2 ).toPrecision1(4); // + ' km';
