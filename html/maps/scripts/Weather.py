...
q3 = "INSERT INTO weather (station_id, weather_json) values('$station_id', '$JSON')";
...
def fmtDate(dt="2015-04-06T22:39:12-04:00"):
    dttm = parse(dt);
    tt = time.mktime(dttm.timetuple())
    tg = time.gmtime(tt)
    t1 = calendar.timegm(dttm.timetuple())
    gmt= datetime.datetime.fromtimestamp(tt)
    loc= datetime.datetime.utcfromtimestamp(tt)
    loc1= datetime.datetime.utcfromtimestamp(t1)
    print tt, tg, t1
    print dt
    print gmt
    print loc
...
def insert(st,dat,jstr,jsn):
    #dt = json["creationDate"];
    if (not dat):
        print "No data for " + jstr;
        return;
    res = runSQL(q3, {"station_id": st, "JSON": jstr })
    print dat, res
...
def run():
    ...
    for i, r in enumerate(rows):
        ...
        dat,jas, jsn = getWeatherLatLon( lt, ln)
        insert(st, dat, jas,jsn);
        #print w;
        if (i == 0):
            break;
